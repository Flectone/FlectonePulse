...
public class RangeFilterImpl implements RangeFilter {
    ...
    @NonNull
    @Override
    public Predicate<FPlayer> createFilter(@NonNull EventMetadata eventMetadata, @NonNull MessageContext messageContext) {
    ...
    @Override
    public boolean checkDistance(@NonNull FPlayer fPlayer, @NonNull FPlayer fReceiver, int range) {
        double distance = platformPlayerAdapter.distance(fPlayer, fReceiver);
        return distance != -1.0 && distance <= range;
    }

    @Override
    public boolean checkWorldNamePermission(@NonNull FPlayer fPlayer, @NonNull FPlayer fReceiver) {
    ...
    @Override
    public boolean checkWorldTypePermission(@NonNull FPlayer fPlayer, @NonNull FPlayer fReceiver) {
