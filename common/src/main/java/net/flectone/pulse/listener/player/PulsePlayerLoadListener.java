...
public class PulsePlayerLoadListener implements PulseListener {
    ...
    public PlayerLoadEvent onPlayerLoadEvent(PlayerLoadEvent event) {
        ...
        boolean hasVanishStatus = socialService.getSetting(fPlayer, SettingText.VANISH_STATUS) != null;
        if (!socialService.hasVanishIntegration()) {
            if (hasVanishStatus) {
                socialService.saveSetting(fPlayer, SettingText.VANISH_STATUS, null);
            }
        } else if (socialService.isVanished(fPlayer) && !hasVanishStatus) {
