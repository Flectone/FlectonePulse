...
public class MaintenanceModuleImpl implements MaintenanceModule {
    ...
    public Set<PermissionSetting> permissions() {
        Set<PermissionSetting> permissions = new LinkedHashSet<>(MaintenanceModule.super.permissions());
        permissions.add(permission().join());
    ...
    public Optional<Moderation> turn(FPlayer fPlayer, @Nullable String reason, long time, boolean turned) {
        ...
        if (!config().filterByServer()) {
            proxySender.send(fTarget, ModuleName.UPDATE_CACHE_MAINTENANCE, dataOutputStream -> dataOutputStream.writeUTF(gson.toJson(moderation)));
        }
