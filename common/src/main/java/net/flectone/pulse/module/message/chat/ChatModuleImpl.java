...
public class ChatModuleImpl implements ChatModule {
    ...
    public Set<PermissionSetting> permissions() {
        Set<PermissionSetting> permissions = new LinkedHashSet<>(ChatModule.super.permissions());
        permissions.addAll(permission().types().values());
        permissions.addAll(permission().types().values().stream().map(Permission.Message.Chat.Type::sound).toList());
        permissions.addAll(permission().types().values().stream().map(Permission.Message.Chat.Type::cooldownBypass).toList());
    ...
    public void sendMessage(FPlayer fPlayer, String rawString, String playerMessage, Chat playerChat) {
        ...
        Set<FPlayer> receivers = messageDispatcher.dispatch(this, EventMetadata.builder()
                ...
                .proxy(dataOutputStream -> {
                    ...
                    dataOutputStream.writeUTF(playerMessage);
                })
