...
public class PulseFixationListener implements PulseListener {
    ...
    public Event onMessageFormattingEvent(MessageFormattingEvent event) {
        ...
        if (!messageContext.isFlag(MessageFlag.FIXATION_MODULE)) return event;
        if (!messageContext.isFlag(MessageFlag.PLAYER_MESSAGE)) return event;

        return event.withContext(fixationModule.format(messageContext));
    }
}
