...
INSERT INTO player_fcolor (player, fcolor, type, number) SELECT s.player, (SELECT id FROM fcolor WHERE name = s.value),'OUT',3 FROM setting s WHERE s.type = 'STYLE' AND s.value != '';

INSERT INTO player_fcolor (player, fcolor, type, number) SELECT s.player,  (SELECT id FROM fcolor WHERE name = s.value),'OUT', 4 FROM setting s WHERE s.type = 'STYLE' AND s.value != '';
