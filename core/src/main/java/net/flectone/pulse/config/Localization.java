...
public final class Localization extends YamlFile implements ModuleConfig {
    ...
    private void initRU_RU() {
        ...
        command.ball.answers.addAll(new LinkedList<>() {
            {
                push("Бесспорно");
                push("Никаких сомнений");
                push("Определённо да");
                push("Это база");
                push("Можешь быть уверен в этом");
                push("Вероятнее всего");
                push("Хорошие перспективы");
                push("Да");
                push("Пока не ясно, попробуй снова");
                push("Спроси позже");
                push("Лучше не рассказывать");
                push("Сейчас нельзя предсказать");
                push("Сконцентрируйся и спроси опять");
                push("Даже не думай");
                push("Нет.");
                push("Перспективы не очень хорошие");
                push("Весьма сомнительно");
            }
        });
