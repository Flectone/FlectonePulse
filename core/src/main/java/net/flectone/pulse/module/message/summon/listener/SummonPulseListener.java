...
public class SummonPulseListener implements PulseListener {
    ...
    @Inject
    public SummonPulseListener(SummonExtractor summonExtractor,
                               SummonModule summonModule) {
        this.summonExtractor = summonExtractor;
        this.summonModule = summonModule;
    }

    @Pulse
    public void onTranslatableMessageReceiveEvent(MessageReceiveEvent event) {
        if (event.getTranslationKey() != MinecraftTranslationKey.COMMANDS_SUMMON_SUCCESS) return;

        Optional<FEntity> target = summonExtractor.extract(event.getTranslatableComponent());
        if (target.isEmpty()) return;

        event.setCancelled(true);
        summonModule.send(event.getFPlayer(), target.get());
    }
}
