    ...
    public void onTranslatableMessageReceiveEvent(MessageReceiveEvent event) {
        ...
        switch (translationKey) {
            case COMMANDS_WORLDBORDER_DAMAGE_AMOUNT_SUCCESS, COMMANDS_WORLDBORDER_DAMAGE_BUFFER_SUCCESS,
                ...
                Optional<Worldborder> optionalWorldborder = worldborderExtractor.extractValue(event.getTranslatableComponent());
                if (optionalWorldborder.isEmpty()) return;

                event.setCancelled(true);
                worldborderModule.send(event.getFPlayer(), translationKey, optionalWorldborder.get());
            }
            case COMMANDS_WORLDBORDER_CENTER_SUCCESS, COMMANDS_WORLDBORDER_SET_GROW,
                 COMMANDS_WORLDBORDER_SET_SHRINK -> {
                Optional<Worldborder> optionalWorldborder = worldborderExtractor.extractSecondValue(event.getTranslatableComponent());
                if (optionalWorldborder.isEmpty()) return;
